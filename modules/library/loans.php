...
<?php
// Handle form submissions BEFORE header to avoid "headers already sent"
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isLoggedIn()) {
    ...
    if (!isset($_POST['csrf_token']) || !validateCSRFToken($_POST['csrf_token'])) {
        // print_r($_POST); die("CSRF FAIL"); // Uncomment to debug CSRF
        $error = "Error de seguridad. Intente nuevamente.";
    } else {
        if (isset($_POST['add_book']) || isset($_POST['submit_add_book'])) {
            ...
            if (empty($title)) {
                ...
                try {
                    ...
                    if ($e->getCode() == 23000) {
                        $error = "Ya existe un libro con ese ISBN. El ISBN debe ser único.";
                    } else {
                        $error = "Error al agregar el libro: " . $e->getMessage();
                    }
                }
            }
        } elseif (isset($_POST['loan_book']) || isset($_POST['submit_loan_book'])) {
        ...
        } elseif (isset($_POST['return_book']) || isset($_POST['submit_return_book'])) {
        ...
        } elseif (isset($_POST['delete_book']) || isset($_POST['submit_delete_book'])) {
            $book_id = (int) $_POST['book_id'];
            ...
            if ($loan_check->fetchColumn() > 0) {
                ...
                try {
                    $pdo->prepare("DELETE FROM book_loans WHERE book_id = ?")->execute([$book_id]);
                    $pdo->prepare("DELETE FROM physical_books WHERE id = ?")->execute([$book_id]);
                    $pdo->commit();
        ...
        } elseif (isset($_POST['admin_loan_book']) || isset($_POST['submit_admin_loan_book'])) {
            $book_id = (int) $_POST['book_id'];
            ...
            if (!$book || $book['available_copies'] <= 0) {
                ...
                $pdo->prepare("UPDATE physical_books SET available_copies = available_copies - 1 WHERE id = ?")->execute([$book_id]);
                $success = "Préstamo registrado.";
            }
        } elseif (isset($_POST['edit_book']) || isset($_POST['submit_edit_book'])) {
            $book_id = (int) $_POST['book_id'];
            $title = sanitize($_POST['title']);
            $author = sanitize($_POST['author']) ?: null;
            $isbn = sanitize($_POST['isbn']) ?: null;
            ...
            // Get current status
            $current_stmt = $pdo->prepare("SELECT total_copies, available_copies FROM physical_books WHERE id = ?");
            $current_stmt->execute([$book_id]);
            $current = $current_stmt->fetch(PDO::FETCH_ASSOC);

            if (!$current) {
                $error = "Error: No se encontró el libro con ID $book_id";
            } else {
                // Check if new total copies is less than minimal needed (total - available = currently loaned)
                $loaned_count = $current['total_copies'] - $current['available_copies'];
                if ($total_copies < $loaned_count) {
                    $error = "No se puede reducir el total de copias a $total_copies porque hay $loaned_count libros actualmente prestados.";
                } else {
                    $new_available = $total_copies - $loaned_count;
                    try {
                        ...
                        if ($e->getCode() == 23000) {
                            $error = "Error: El ISBN ya está en uso por otro libro.";
                        } else {
                            $error = "Error al actualizar: " . $e->getMessage() . " (Código: " . $e->getCode() . ")";
                        }
                    }
                }
            }
...
// Data fetching for display - consolidated here after processing
if (isLoggedIn()) {
    ...
    // User's loans
    $stmt = $pdo->prepare("SELECT bl.*, pb.title, pb.author, pb.isbn FROM book_loans bl JOIN physical_books pb ON bl.book_id = pb.id WHERE bl.user_id = ? ORDER BY bl.loan_date DESC");
    $stmt->execute([$_SESSION['user_id']]);
    $user_loans = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // All loans for admin
    if (getUserRole() == 'admin') {
        $all_loans = $pdo->query("SELECT bl.*, pb.title, pb.author, pb.isbn, u.name as user_name FROM book_loans bl JOIN physical_books pb ON bl.book_id = pb.id JOIN users u ON bl.user_id = u.id ORDER BY bl.loan_date DESC")->fetchAll(PDO::FETCH_ASSOC);
    }
}
?>
            ...
            <form method="GET" class="grid md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="search_author" name="search_author"
                        value="<?php echo htmlspecialchars($_GET['search_author'] ?? ''); ?>"
                ...
                <div class="flex items-end">
                    <button type="submit"
                        class="bg-gradient-to-r from-primary to-secondary text-white font-bold py-3 px-6 rounded-lg hover:shadow-lg transition duration-300 transform hover:scale-105 flex items-center mr-2">
                        <i class="fas fa-search mr-2"></i>
                        Buscar
                    </button>
                    <a href="loans.php"
                        class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-3 px-6 rounded-lg transition duration-300 flex items-center">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-xl animate-fade-in-up">
                ...
                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($filtered_books as $book): ?>
                        <?php if ($book['available_copies'] > 0 && $book['status'] == 'available'): ?>
                            <div
                                class="bg-gray-50 p-6 rounded-xl border-2 border-gray-200 hover:border-primary transition duration-300">
                                <div class="flex items-start justify-between mb-4">
                                    <div class="flex-1">
                                        <h4 class="text-lg font-bold text-gray-800 mb-2"><?php echo htmlspecialchars($book['title']); ?>
                                        </h4>
                                ...
                                <?php if ($book['description']): ?>
                                    <p class="text-gray-600 text-sm mb-4 line-clamp-2"><?php echo htmlspecialchars($book['description']); ?>
                                    </p>
                                <?php endif; ?>

                                <form method="POST"
                                    onsubmit="return confirm('¿Solicitar préstamo de este libro? Tendrá 14 días para devolverlo.')">
                                    <input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>">
                                    <input type="hidden" name="book_id" value="<?php echo $book['id']; ?>">
                                    <input type="hidden" name="submit_loan_book" value="1">
                                    <button type="submit" name="loan_book"
                                        class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-6 rounded-lg transition duration-300 transform hover:scale-105 flex items-center justify-center">
            ...
            <?php else: ?>
                <div class="text-center py-8">
                    ...
                    <a href="loans.php"
                        class="mt-4 inline-block bg-primary hover:bg-yellow-600 text-white font-bold py-3 px-6 rounded-lg transition duration-300">
                        <i class="fas fa-times mr-2"></i>
                        Limpiar filtros
                    </a>
                </div>
    ...
    <?php if (isLoggedIn() && getUserRole() == 'admin'): ?>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-xl mb-8 animate-fade-in-up">
            ...
            <form method="POST" class="space-y-6">
                <input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>">
                <input type="hidden" name="submit_add_book" value="1">
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="total_copies" class="block text-sm font-medium text-gray-700 mb-2">Total de
                            Copias</label>
                    ...
                    <div>
                        ...
                        <input type="text" id="location" name="location" placeholder="Ej: Estante A-1"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition duration-200">
                    </div>
                </div>
                <div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php foreach ($books as $book): ?>
                            <tr class="hover:bg-gray-50 transition duration-200">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    <span
                                        class="font-medium"><?php echo $book['available_copies']; ?>/<?php echo $book['total_copies']; ?></span>
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <span
                                        class="px-2 py-1 text-xs rounded-full
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex space-x-2">
                                        <button
                                            onclick="openLoanModal(<?php echo $book['id']; ?>, '<?php echo addslashes($book['title']); ?>')"
                                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-3 rounded-lg transition duration-200"
                                            title="Prestar libro">
                                            <i class="fas fa-hand-holding mr-1"></i>
                                            Prestar
                                        </button>
                                        <button onclick="editBook(<?php echo $book['id']; ?>)"
                                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-3 rounded-lg transition duration-200">
                                            <i class="fas fa-edit mr-1"></i>
                                            Editar
                                        </button>
                                        <form method="POST" class="inline"
                                            onsubmit="return confirm('¿Eliminar este libro? Asegúrese de que no tenga préstamos activos.')">
                                            <input type="hidden" name="csrf_token" value="<?php echo generateCSRFToken(); ?>">
                                            <input type="hidden" name="book_id" value="<?php echo $book['id']; ?>">
                                            <input type="hidden" name="submit_delete_book" value="1">
                                            <button type="submit" name="delete_book"
                                                class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-3 rounded-lg transition duration-200">
                ...
                <table class="min-w-full table-auto">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php foreach ($all_loans as $loan): ?>
                            <tr class="hover:bg-gray-50 transition duration-200">
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                    <?php echo htmlspecialchars($loan['user_name']); ?>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                    <?php echo htmlspecialchars($loan['title']); ?>
                                    <?php if ($loan['author']): ?>
                                        <br><small class="text-gray-500">por
                                            <?php echo htmlspecialchars($loan['author']); ?></small>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600">
                                    <?php echo htmlspecialchars($loan['isbn'] ?? 'N/A'); ?>
                                    ...
                                    <div class="flex space-x-2">
                                        <?php if ($loan['status'] == 'active' || $loan['status'] == 'overdue'): ?>
                                            <form method="POST" class="inline"
                                                ...
                                                <input type="hidden" name="submit_return_book" value="1">
                                        ...
                                        <?php endif; ?>
                                        <button onclick="viewLoanDetails(<?php echo $loan['id']; ?>)"
                                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-3 rounded-lg transition duration-200">
    ...
    <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-2xl shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form method="POST" id="loanForm">
                ...
                <input type="hidden" name="submit_admin_loan_book" value="1">
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        <label for="due_date" class="block text-sm font-medium text-gray-700 mb-2">Fecha de
                            Devolución</label>
                    ...
                    <div>
                        ...
                        <input type="text" name="notes" id="notes" placeholder="Observaciones del préstamo"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition duration-200">
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit" name="admin_loan_book"
                        class="bg-primary hover:bg-yellow-600 text-white font-bold py-2 px-6 rounded-lg transition duration-300 transform hover:scale-105">
    ...
    <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form method="POST" id="editForm">
                ...
                <input type="hidden" name="submit_edit_book" value="1">
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        <label for="editTotalCopies" class="block text-sm font-medium text-gray-700 mb-2">Total de
                            Copias</label>
                        <input type="number" id="editTotalCopies" name="total_copies" min="1" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition duration-200">
                    </div>
                    <div>
                        <label for="editLocation" class="block text-sm font-medium text-gray-700 mb-2">Ubicación</label>
                        <input type="text" id="editLocation" name="location" placeholder="Ej: Estante A-1"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition duration-200">
                    </div>
                    <div>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="space-y-2 text-sm">
                                        ...
                                        <p><span class="font-medium">Estado:</span>
                                            <span class="px-2 py-1 text-xs rounded-full ml-2 ${loan.status == 'active' ? 'bg-green-100 text-green-800' :
                    (loan.status == 'returned' ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800')
                }">
                            ...
                            <div class="bg-yellow-50 p-4 rounded-lg">
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 rounded-full ${loan.status == 'active' ? 'bg-green-500' :
